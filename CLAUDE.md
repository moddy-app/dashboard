...
- **Notifications** — boîte de réception du compte connecté (menu utilisateur → Notifications, ⌘K, pastille de non-lues). Ce n'est **pas** un module : le système est centralisé et partagé avec le bot, qui écrit dans quatre tables PostgreSQL dont il est propriétaire — le dashboard **ne fait que lire**. Cinq points portés par le code. **1. Le contenu stocké est un *gabarit*, pas un message** : ses chaînes portent encore `{user}`, `{server}` ; dix mille destinataires du même DM partagent **une** ligne de contenu, chaque `variables` disant ce qui a été substitué pour une personne. L'API *devrait* servir du résolu (`content`), mais si elle laisse passer le `payload` brut, `normalizeContent()` applique l'algorithme lui-même — un écran plein d'`{accolades}` n'est jamais acceptable. La substitution (`substitute()` dans `src/lib/notifications.ts`) doit correspondre **au caractère près** à celle du bot : clé absente → placeholder **laissé visible** (c'est ainsi qu'un gabarit cassé se remarque), `null` → chaîne vide, sémantique de `str()` en Python (`True`, pas `true` — `variables` est du JSONB), aucune récursion, et jamais un moteur de template qui jette sur une accolade orpheline. **2. Le texte vient d'un admin de serveur** : il passe par `DiscordMarkup` (rendu contrôlé), jamais par `dangerouslySetInnerHTML` ; les URL de `links` sont écartées si elles ne sont pas `https://` **ou** si elles portent encore un placeholder non substitué. La syntaxe que seul Discord sait rendre (`<@123>`, `<@&123>`, `<#123>`, `<t:…:R>`) **dégrade en texte** (`degradeDiscordSyntax()`) — seule la mention du lecteur est résolue, c'est la seule identité sous la main et de loin la plus fréquente. Les émojis custom `<:nom:id>` deviennent une URL CDN, jamais la balise brute ; `accent_color` est un **entier** décimal (`5793266` → `#5865F2`), pas une chaîne CSS. **3. L'origine se lit comme dans Discord** (§6.4 du guide) : le serveur avec son icône, son nom et sa coche, sinon le libellé du service, et **rien du tout** quand `kind = 'official'` — Moddy en tant qu'institution n'a pas de tiers à nommer. Le registre des services est **ouvert** : un `source_service` inconnu se dégrade en « Moddy », jamais en clé nue ni en erreur (même règle pour `kind`, `author`, `status` — le bot livre de nouveaux expéditeurs sans déploiement du back-end). **4. `platforms` est une intention, `notification_deliveries` un fait** : un `discord_status` `failed`/`skipped` est **affiché** (« Moddy n'a pas pu vous envoyer ce message ») — c'est précisément ce qui justifie une boîte de réception sur le dashboard. **5. Aucun accusé de lecture n'existe côté API** (le guide interdit d'ajouter une colonne aux quatre tables) : le lu/non-lu vit dans `localStorage` (`moddy_notifications_read`), avec une borne temporelle pour « tout marquer comme lu » plutôt qu'une énumération d'ids — les pages non encore chargées sont couvertes elles aussi. C'est un confort d'affichage, pas une donnée de référence. Pagination **keyset** (`?before=<created_at>,<uuid>`), jamais d'`OFFSET`. La locale du **chrome** vient de `notification.locale` (la langue dans laquelle le message a été rendu), pas de celle du lecteur : c'est l'erreur que cette colonne existe pour éviter. **Pas de bouton « signaler »** : `reportable` est gelé à l'envoi et ne se recalcule ni ne s'élargit, mais déposer un signalement publie aussi le panneau de revue Discord et le journalise — le déclencheur n'existe pas encore côté bot, donc c'est une information qu'on affiche, pas une action qu'on offre. Types dans `src/types/notifications.ts`, helpers `src/lib/notifications.ts` et `src/lib/notification-read-state.ts`, service `src/services/notifications.ts`, hook `src/hooks/useNotifications.ts`, composant `src/components/notification-drawer.tsx`.
...
*Dernière mise à jour : 2026-08-26 (notifications branchées sur l'API)*
