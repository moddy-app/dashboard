...
- **Brocoli — assistant IA** (`/servers/:guildId/brocoli`) — conversation qui lit et configure le serveur. **`EventSource` ne convient pas** : les endpoints de tour sont des `POST` à corps JSON qui répondent en `text/event-stream` → client SSE écrit à la main sur `fetch` + `ReadableStream` (`src/lib/ai-stream.ts`), avec **report du tampon incomplet** (un événement peut être coupé entre deux paquets) et `parseApiJson` plutôt que `JSON.parse` (les snowflakes du flux dépassent 2^53). Corollaire : **aucune reconnexion, aucun rejeu** d'un envoi en échec — une coupure se répare par `GET /ai/conversations/{id}`. Un tour peut s'arrêter **sans être terminé**, de deux façons qu'il ne faut pas confondre : `awaiting_confirmation` (une **confirmation** — Brocoli sait quoi faire et demande la permission d'agir → carte + « Appliquer », panneau **épinglé au-dessus de la saisie**, jamais dans le fil où il défilerait hors de l'écran ; risque en `Badge` stock, `critical` exigeant un geste délibéré même en mode `auto` ; `preview.valid === false` grise « Appliquer » ; `diff` absent → on le **dit**, on n'invente pas d'aperçu) et `awaiting_answer` (une **question** — il lui manque une information et ne peut pas continuer → **formulaire**). Les cinq statuts de `run_end` se traitent explicitement : un `default` qui rend la main à la saisie ferait sortir de l'attente sans rien afficher. Le **formulaire de question** est le seul endroit où une omission est silencieuse : sans lui, le tour est fini, aucun texte n'est arrivé, et l'écran reste muet — il s'affiche donc **dans les trois modes, `auto` compris**. `recommended` est une **valeur par défaut présélectionnée**, pas un indice (Brocoli a lu le serveur avant de demander) ; `null` = ne rien fabriquer à sa place ; `recommended_label` ne sert qu'à garder sélectionnable un identifiant absent des listes. Un **widget natif par `answer_type`** (les vrais sélecteurs de salon et de rôle — un champ texte de repli annulerait l'intérêt de la fonctionnalité), `header === ''` → **pas de puce** (le backend le vide exprès : une puce dans la mauvaise langue est pire que pas de puce), bouton **« Ignorer »** (`{cancelled: true}`, sans quoi la seule sortie serait de répondre n'importe quoi), `label`/`labels` **renvoyés** (Brocoli les réemploie au lieu de relire les salons), champ vide = `skipped`, envoi verrouillé dès le premier clic (deux flux SSE dont le second recevrait `409`). La réponse est **un nouveau flux**, consommé par le même gestionnaire — il s'achève souvent sur une `permission_request`. À la relecture, une question répondue laisse **deux lignes** (`pending` puis `answered`/`cancelled`) : `itemsFromTranscript()` les regroupe par `question_id` — place de la première, état de la dernière — et l'`expires_at` de la ligne est comparé à l'heure courante avant de reproposer le formulaire (sinon `409`). Dégradations : `answer_type` inconnu → `choice`/`text` (jamais rien), statut inconnu → **réglé**, `risk` inconnu → `high` (jamais `low`). Erreurs : toutes **avant** le flux, en JSON ; `403`/`404` **ferment** la conversation, `409` se répare en relisant l'état (jamais en boucle), `429` lit `Retry-After`, le message du backend fait foi quand il en porte un. Le texte de Brocoli n'est **jamais** rendu en HTML brut (markdown restreint parsé en nœuds React) ; il écrit `#salon` / `@rôle` avec les vrais noms — pastilles locales, un nom inconnu reste du texte. On insère le **nom** dans la saisie, pas `<#123>` : Brocoli lit du texte et résout lui-même. Les titres de conversation sont générés côté backend en tâche de fond : **repli tant que `title` est `null`** (ce n'est pas une erreur), jamais régénérés côté client. Types `src/types/ai.ts`, client SSE `src/lib/ai-stream.ts`, helpers `src/lib/brocoli.ts` et `src/lib/brocoli-mentions.ts`, service `src/services/ai.ts`, hooks `src/hooks/useBrocoli.ts` / `useAiStatus.ts` / `useCountdown.ts`, composants `src/components/brocoli/`, page `src/pages/BrocoliPage.tsx`.
...
| `/servers/:guildId/brocoli` | `BrocoliPage` | Oui | Brocoli, l'assistant IA de configuration |
...
*Dernière mise à jour : 2026-08-29 (Brocoli : formulaire de question)*
